    ...
    actions: {
        ...
        async QuerySynchronizationFeedback({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySynchronizationFeedback', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySynchronizationFeedbackAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).querySynchronizationFeedbackAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySynchronizationFeedbackAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySynchronizationRequest({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySynchronizationRequest', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QuerySynchronizationRequestAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).querySynchronizationRequestAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySynchronizationRequestAll', payload: { options: { all }, params: { ...key }, query } });
