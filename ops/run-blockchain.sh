...
docker exec first_node_starport_1 sed -i 's/persistent_peers = ".*/persistent_peers = "'${node2_id}'@'${internal_host_ip}':'${second_node_tendermint_p2p_port}'"/' ~/.baseledger/config/config.toml
docker exec second_node_starport_1 sed -i 's/persistent_peers = ""/persistent_peers = "'${node1_id}'@'${internal_host_ip}':'${first_node_tendermint_p2p_port}'"/' ~/.baseledger/config/config.toml

# this enables grpc
docker exec first_node_starport_1 sed -i 's@laddr = "tcp://127.0.0.1:'${tendermint_internal_grpc_port}'"@laddr = "tcp://0.0.0.0:'${tendermint_internal_grpc_port}'"@' ~/.baseledger/config/config.toml
docker exec second_node_starport_1 sed -i 's@laddr = "tcp://127.0.0.1:'${tendermint_internal_grpc_port}'"@laddr = "tcp://0.0.0.0:'${tendermint_internal_grpc_port}'"@' ~/.baseledger/config/config.toml
